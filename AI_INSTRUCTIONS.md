...
- `tsconfig.json`: Strict TypeScript (strict:true) with ES2022 + NodeNext module resolution.
...
- `rename_node`, `delete_node`, `duplicate_node` (supports `parentId` to duplicate into different parent)
- `resize_node`, `rotate_node`, `set_position`
- `move_to_parent`: move node to different parent container (frame, group, component, page)
- `reorder_node`: change z-order/stacking position among siblings
- `get_node_info`: get detailed info (parent, bounds, children, visibility, locked state)
- `set_visibility`, `set_locked`: show/hide and lock/unlock nodes
- `flatten_node`: flatten complex nodes to single vector
- `group_nodes`, `ungroup`
